...
bash scripts/run_demo.sh --smoke --output results/demo_smoke
...
`bash scripts/run_demo.sh --help` (and the train/eval wrappers) lists flags.
...
```bash
python scripts/download_checkpoints.py                  # GAE-64 codec + flow + stats
python scripts/download_checkpoints.py --list
python scripts/download_checkpoints.py --subset codec
```
...
```bash
python scripts/prepare_data.py re10k --source /datasets/RealEstate10K --output "$GAE_DATA_ROOT/re10k_packed" --split train --workers 8
python scripts/prepare_data.py dl3dv --source /datasets/DL3DV-10K --output "$GAE_DATA_ROOT/dl3dv_packed" --workers 8
python scripts/preprocess_scannetpp.py --source /datasets/scannetpp --output "$GAE_DATA_ROOT/scannetpp_preprocessed" --num-shards 8 --shard-index 0
python scripts/preprocess_mvssynth.py  --source /datasets/MVS-Synth/GTAV_540 --output "$GAE_DATA_ROOT/mvssynth_packed"
...
```bash
python scripts/prepare_t2i_data.py blip3o   --output "$GAE_DATA_ROOT/BLIP3o" --splits long short journeydb
python scripts/prepare_t2i_data.py imagenet --mode arrow --output "$GAE_DATA_ROOT/imagenet-1k"   # gated: huggingface-cli login
...
python scripts/finetune_geo_decoder.py --config configs/gae_128.yaml \
...
python scripts/merge_geoft_vae_ckpt.py --geoft results/geoft/run0/geoft_002000.pt \
...
```bash
python scripts/eval_reconstruction.py --config configs/gae_64.yaml --vae-ckpt ckpts/gae_64.pt --dataset re10k_packed --data-root "$GAE_DATA_ROOT/re10k_packed/test"
python scripts/encode_latents.py --input "$GAE_DATA_ROOT/re10k_packed/test" --config configs/gae_64.yaml --codec-ckpt ckpts/gae_64.pt --output results/gae64_latents.pt
python scripts/eval_latent.py --latents gae64=results/gae64_latents.pt --json results/gae64_latent_metrics.json
```
...
```bash
python scripts/generate.py --image examples/scenes/forest_lake_trail.jpg --prompt-file examples/scenes/forest_lake_trail.txt --hf-repo TencentARC/GAE-D64-1B --output results/demo
```

Pure prompt -> image + depth + point cloud (single frame, via the codec RGB + DPT heads):

```bash
python scripts/generate_t2i.py --hf-repo TencentARC/GAE-D64-1B --prompts-file examples/t2i_prompts.txt --output results/t2i
```
...
python scripts/eval_generation.py --config configs/flow_gae64.yaml \
...
python scripts/eval_3d_consistency.py --pred-dir results/gen_gae64 \
  --dataset re10k_packed --data-root "$GAE_DATA_ROOT/re10k_packed" \
  --vggt-ckpt ckpts/vggt.pt --da3-ckpt ckpts/da3_giant.pt \
  --output-name results/gen_gae64/consistency.json
python scripts/eval_met3r.py --pred-dir results/gen_gae64 --output results/gen_gae64/met3r.json

# Table 7 (depth/point-map/pose from sampled latents); needs Pi3
python scripts/eval_geometry.py --geom-dir results/gen_gae64 \
...
One-click: `scripts/run_eval.sh --size 64 --tasks gen,consistency,met3r,geometry`.
