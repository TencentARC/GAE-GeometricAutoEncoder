...
### 🔁 VAE reconstruction (RGB + depth + point cloud)

The reconstruction demo encodes each input video with the released codec and
decodes RGB, viridis-colored depth, and a point cloud from the predicted depth
and rays. Run the complete named scene gallery:
...
- **VAE Reconstruction (Codec)**: reconstructs the named videos in
  ...
  provides the decoded point cloud as a downloadable PLY.
