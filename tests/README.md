...
| `test_pipeline_smoke.py` | `scripts/smoke_test.py` runs the stubbed end-to-end pipeline with no downloads (needs `torch`; skipped otherwise) |
| `test_prepare_data.py` | `scripts/prepare_data.py` emits the packed `video.mp4` + `meta.json` schema the loaders expect, with integer-aware frame ordering (needs `ffmpeg` + `cv2`; skipped otherwise) |
